...
- **Live print notifications for Android and iOS.** One card per printer that updates in place -- on the iOS Lock Screen and in the Dynamic Island, and in the Android status bar and shade -- with a countdown timer to the estimated finish, a progress bar, the current layer, and the G-code preview as its icon. It ends itself when the print does, replaced by a completion notification carrying a camera snapshot of the bed.
  - The countdown **runs on the phone**, so it costs no pushes at all. A state change (start, pause, resume, finish) is pushed immediately; the rest is rate limited -- a five-minute refresh, a whole percent of progress to force an earlier one, and a 30-second floor under those -- because iOS throttles frequent Live Activity updates and eventually drops them.
...
- **A cancelled or aborted print now says so.** The live card was dismissed and nothing else happened, so from the phone's side a notification silently disappeared with no indication of whether the print had finished or died. There is a new banner naming the file and the percentage it stopped at, plus a `ha_creality_ws_print_stopped` bus event. It catches the stop **however it was made** -- from Home Assistant, from the Creality app, or from the printer's own screen. None of those announces itself in telemetry: the printer returns to the same state it uses for warming up, with the file name still attached and the progress reset to 0, so the end is only visible as a transition. The file name and percentage quoted are the ones from the last frame that showed the job actually running. It shares the *Notify when a print ends* toggle, because being told a print completed is only half the story.
- **Write your own notification text.** *Configure → Notifications → Custom text* takes a template for any of the six notifications this integration composes: the live card, print finished, print stopped, finishing soon, printer error and filament runout. Leave a field empty, or clear it again, to go back to the built-in translated wording.
  - Placeholders are single-braced and come from the printer's own telemetry, read from the one frame that triggered the notification: `{device}`, `{filename}`, `{progress}`, `{layer}`, `{total_layers}`, `{eta}`, `{elapsed}`, `{filament}`, `{nozzle}`, `{bed}`, `{state}`, `{error_code}`, `{error_key}` and `{minutes}`. Nothing is estimated or carried over from an earlier job, so there is no placeholder for a value the printer does not send -- filament *weight*, for one, which would need a density it never reports.
  - **Not every notification can fill every placeholder**, and each field lists the ones it accepts. Only the error notification knows an error code and only the finishing-soon reminder knows the minutes left; less obviously, *print finished* has no `{eta}`, and *print stopped* offers only `{device}`, `{filename}`, `{progress}`, `{nozzle}` and `{bed}`, because by the frame a stop becomes visible the printer has already reset the progress, the job clock, the layer and the filament length.
  - **Optional sections in square brackets** disappear whole while anything inside them is unknown, so `{filename} -- {progress}%[ -- {eta} left]` loses the tail in the first minute of a job instead of rendering a dangling separator. Brackets around no placeholder are your own text; double them (`[[`, `]]`) for a bracket you want to keep. A `0` counts as a value, not a gap.
  - A placeholder the notification cannot fill is **refused when you submit the page**, which is the only moment a typo can be shown to the person who made it -- a notification is composed on a WebSocket frame, where the log is the only recourse.
  - The **title is always the printer's name**. Android fixes a live card's title when the activity starts and cannot change it afterwards, so job detail belongs in the body. For control over the title too, use the bus events.
...
- **The built-in notification wording is shorter and leads with the printer.** `Print "3DBenchy.gcode" completed successfully!` became `K1C finished.`, and the other five messages follow the same shape. The title already carries the printer's name and the live card already shows the file, so repeating both in the body crowded out what had actually changed -- and the body is what a phone truncates. (Along the way the old single quotes around the file name went too: Home Assistant's translation validation rejects placeholders inside them.) If you preferred the old phrasing, *Custom text* above reproduces it exactly: `Print {filename} completed successfully!`.
- **Each options page saves when you submit it.** The dialog used to stage every page and write them all from a *Save and apply* menu item -- the one entry you could not skip, and it did not look like one, so closing the dialog the way a settings dialog normally closes threw the lot away. Submitting a page now saves that page and reloads the integration to apply it; *Done* only closes the dialog, and so does the window's own close button. The back arrow still abandons the page it is in.
- **Changing only notification settings no longer reloads the integration.** Those options are adopted in place, so the WebSocket connection stays up and no entity blinks unavailable while you tune notification text. Anything else -- camera, power switch, connection -- still reloads, and so does an options update that touches nothing the integration recognises.
...
- **Late-discovered entities were scheduled from the wrong thread.** When the printer reports a CFS unit or a chamber after setup, the sensor platform's handler runs as a Home Assistant job -- and a plain synchronous one is run in an executor thread. It called `hass.loop.call_soon`, which is not thread-safe, so creating those entities raced the event loop. The handler is now marked as a callback and runs on the loop, which is also where it was always meant to be: it does no blocking work.
- **A printer reporting a bare error code took every print-status attribute down with it.** Most firmware sends `err` as `{errcode: N}`, some sends the number on its own, and the state derivation has always handled both. The attribute dict did not: it called `.get()` on whatever arrived, so a bare code -- or an explicit `null` -- raised `AttributeError` while the attributes were being built, and the sensor lost `state_raw`, `left_time_s` and the rest along with `error_code`.
...
- **Wrong CFS spool colour: leading pad character was kept** (closes #113 and #117 part 1, thanks @raf802):
  - Creality RFID tags store the colour as *seven* hex characters -- one padding character followed by the real `RRGGBB` -- and the printer streams that verbatim. Reading the first six digits produced the wrong colour, e.g. the tag's `0ffffff` was reported as `#0fffff` instead of `#ffffff`.
...
- **Completion notification only ever arrived once per file name**: the "already notified" flag was only cleared when the print file name changed, so reprinting the same file never notified again. It is now also re-armed once progress falls back below `NOTIFY_REARM_PROGRESS_MAX` (90%), or -- for a job that never got near the end -- when the printer's job clock restarts. Re-arming on any dip below 100% would have fired twice per print, because the printer rounds up to 100 a second early and then reports 99 once more. Found while verifying #112 against a live printer.
...
- `LATE_DISCOVERY_FIELDS` in `const.py` lists the telemetry fields that gate entity creation (`boxsInfo`, `boxTemp`, `maxBoxTemp`, `targetBoxTemp`); the coordinator fires a single discovery signal the first time each appears, replacing the CFS-only trigger. Platforms subscribe and re-check idempotently.
